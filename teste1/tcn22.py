...
num_samples = 500   #Nº total de amostras usadas para o gráfico gerado.
train_batches = 20   #Nº de batches usados no conjunto de treinamento.
val_batches = 2      #Nº de batches no conjunto de validação. 
seq_length = 500     #comp. de cada sequência de entrada e saída usada no modelo do sistema real.

#Parâmetros de ruído
sigma_v = 0.3 #Desvio padrão do ruído de processo.
sigma_w = 0.3 #Desvio padrão do ruído de medição. 

def generate_data_with_past_y(batches, seq_length):
    u, y = generate_data(batches, seq_length)
    y_past = np.roll(y, shift=1, axis=1)  # Desloca `y` para trás no tempo
    y_past[:, 0] = 0  # Define o primeiro valor como zero (ou algum valor neutro)
    return np.stack([u, y_past], axis=1), y  # Retorna (entrada com 2 canais, saída)
...
u_y_train, y_train = generate_data_with_past_y(train_batches, seq_length)#
u_y_val, y_val = generate_data_with_past_y(val_batches, seq_length)

# Converter para tensores
u_y_train_tensor = torch.tensor(u_y_train, dtype=torch.float32)
y_train_tensor = torch.tensor(y_train, dtype=torch.float32).unsqueeze(1)

u_y_val_tensor = torch.tensor(u_y_val, dtype=torch.float32)
...
# Criar o modelo. Agora usa 2 canais
model = TCN(input_size=2, output_size=1, num_channels=16, kernel_size=kernel_size, dropout=dropout_rate)
...
for epoch in range(epochs):
    ...
    outputs = model(u_y_train_tensor) #Passa os dados de entrada pelo modelo para obter as previsões.
    ...
    val_outputs = model(u_y_val_tensor) #
...
model.eval()
u_sim_y_sim, y_sim = generate_data_with_past_y(1, seq_length)  # Gera entrada com 2 canais
u_sim_tensor = torch.tensor(u_sim_y_sim, dtype=torch.float32)  # Converte corretamente
